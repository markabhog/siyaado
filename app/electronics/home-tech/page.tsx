...
export default function HomeTechPage() {
  ...
  const [homeTechProducts, setHomeTechProducts] = useState<HomeTechProduct[]>([]);
  ...
  const fetchHomeTechData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/products?category=home-tech');
      
      if (response.ok) {
        const data = await response.json();
        const products = Array.isArray(data) ? data : (data.products || []);
        
        const transformedProducts: HomeTechProduct[] = products.map((product: any) => {
          ...
          return {
            ...
            type: attributes?.type || 'Home Tech',
            connectivity: attributes?.connectivity || 'WiFi',
            compatibility: attributes?.compatibility || 'Universal',
        ...
        setHomeTechProducts(transformedProducts);
      } else {
        setHomeTechProducts(getMockHomeTechProducts());
      }
    } catch (error) {
      console.error('Error fetching home tech data:', error);
      setHomeTechProducts(getMockHomeTechProducts());
    } finally {
      setLoading(false);
    }
  ...
  const getMockHomeTechProducts = (): HomeTechProduct[] => [
    {
      id: '1',
      ...
      features: ['Voice Control', 'App Control', 'Automation'],
      isFeatured: true,
      discount: 20,
    ...
    },
    {
      ...
      type: 'Thermostat',
      connectivity: 'WiFi',
      features: ['Energy Saving', 'Learning Mode', 'Remote Control'],
      isNew: true,
    ...
    },
    {
      ...
      type: 'Lighting',
      connectivity: 'WiFi',
      features: ['Color Changing', 'Dimmable', 'Timer'],
      isFeatured: true,
      description: 'Smart LED bulbs with color changing and dimming',
      warranty: '1 year',
      color: 'Multi'
    }
  ];

  const filteredHomeTechProducts = homeTechProducts.filter(product => {
  ...
  const sortedHomeTechProducts = [...filteredHomeTechProducts].sort((a, b) => {
    switch (sortBy) {
      case 'price-low':
  ...
  const HomeTechProductCard = ({ product, index, viewMode }: { product: HomeTechProduct; index: number; viewMode: 'grid' | 'list' }) => (
    <motion.div
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-slate-600">Loading home tech products...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="relative max-w-7xl mx-auto px-4">
          ...
          >
            ...
            <p className="text-xl text-slate-600 mb-8 max-w-3xl mx-auto">
              Transform your home with smart technology including hubs, thermostats, lighting, and automation systems.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-12">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="text-3xl font-bold text-slate-900 mb-2">Home Tech Products</h2>
              <p className="text-slate-600">Smart home solutions for modern living</p>
            </div>
          ...
          {homeTechProducts.length === 0 ? (
            <div className="text-center py-12">
              <div className="text-6xl mb-4">🏠</div>
              <h3 className="text-xl font-semibold text-slate-900 mb-2">No home tech products found</h3>
              <p className="text-slate-600">Check back later for amazing smart home products!</p>
            </div>
          ) : (
            <div className={'grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'}>
              <AnimatePresence>
                {sortedHomeTechProducts.map((product, index) => (
                  <motion.div
                    key={product.id}
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            <div>
              ...
              <p className="text-slate-400 mb-4">
                Your trusted destination for smart home technology and automation products.
              </p>
            ...
            <div>
              <h4 className="font-semibold mb-4">Home Tech Categories</h4>
              <ul className="space-y-2 text-slate-400">
                <li><Link href="/electronics/home-tech" className="hover:text-white">Smart Hubs</Link></li>
                <li><Link href="/electronics/home-tech" className="hover:text-white">Thermostats</Link></li>
                <li><Link href="/electronics/home-tech" className="hover:text-white">Lighting</Link></li>
                <li><Link href="/electronics/home-tech" className="hover:text-white">Security</Link></li>
              </ul>
            </div>
            <div>
